(function () {
    ...
    class RgsSelector extends BaseComponent {
        ...
        syncFromParent(nextProps) {
            if (!nextProps) { return; }
            const updates = {};
            if (Object.prototype.hasOwnProperty.call(nextProps, 'value')) {
                const nextValue = (nextProps.value || '').toString();
                if (nextValue !== (this.state.query || '')) {
                    updates.query = nextValue;
                }
            }
            if (Object.prototype.hasOwnProperty.call(nextProps, 'selected')) {
                const nextSelected = nextProps.selected || null;
                if (!this.isSameItem(nextSelected, this.state.selected)) {
                    updates.selected = nextSelected;
                }
            }
    ...
    class IntranetApp extends BaseComponent {
        constructor(props) {
            ...
            this.rgsSelector = new RgsSelector({
                value: this.state.accountForm.rgs_code,
                selected: this.state.accountForm.selectedRgs,
                onSelect: (item) => this.updateAccountForm({ rgs_code: item.code, selectedRgs: item })
            });
            this.rgsSelector.el = this.rgsSelector.render();
        }
        ...
        renderAccountForm() {
            ...
            if (this.rgsSelector) {
                this.rgsSelector.syncFromParent({
                    value: form.rgs_code,
            ...
            return h('form', {
                ...
                this.rgsSelector ? this.rgsSelector.el : null,
                h('label', null, [
                    'Rekeningtype',
                    h('select', {
                        name: 'account_type',
